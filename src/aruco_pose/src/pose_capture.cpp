...
#include <aruco_pose/pose_capture.hpp>
  ...
  LOGGER(this->get_logger())
{
  ...
  camera_subscription_ = this->create_subscription<sensor_msgs::msg::Image>(
    ...
    std::bind(&PoseCapture::image_raw_callback, this, std::placeholders::_1));
...
void PoseCapture::image_raw_callback(
  const sensor_msgs::msg::Image::ConstSharedPtr & rgb_msg)
{
  ...
  //   outputImage, markerCorners_, markerIds_);
  ...
  try {
    if (!markerIds_.empty()) {
      ...
      for (unsigned int i = 0; i < markerIds_.size(); i++) {
        cv::drawFrameAxes(
          outputImage, cameraMatrix_, distCoeffs_, rvecs[i], tvecs[i],
          physical_marker_size_ * 2.0f, 12);

        // Draw marker IDs with custom font size
        cv::Point2f textPosition = markerCorners_[i][0]; // Top-left corner
        std::string text = "id:" + std::to_string(markerIds_[i]);
        cv::putText(
          outputImage, text, textPosition, cv::FONT_HERSHEY_SIMPLEX, 3.0, cv::Scalar(
            0, 0,
            255), 4.5);
      }

      std::string filename_rgb = "data/rgb/" + std::to_string(rgb_msg->header.stamp.sec) + ".png";
      RCLCPP_INFO(LOGGER, "About to save images at %s.", filename_rgb.c_str());
...
geometry_msgs::msg::Quaternion PoseCapture::toQuaternion(double roll, double pitch, double yaw)
{
...
int main(int argc, char ** argv)
{
  ...
  auto aruco_pose_node = std::make_shared<PoseCapture>();
